...
// Three-input variant of the fused add+norm below: the Laguna layer tail sums
// residual, routed, and shared branches, and the sum's RMSNorm is always the
// next consumer, so folding the two saves a dispatch and a round trip per
// layer.  The addition keeps kernel_add3_f32's (a + b) + c order and the
// reduction reuses this kernel family's geometry, so both outputs match the
// separate dispatches bit for bit.
kernel void kernel_add3_rms_norm_mul_f32_4(
    ...
    float sumf = 0.0f;
    for (int i00 = tpitg.x; i00 < args.ne00_t; i00 += ntg.x) {
        const float4 v = a[i00] + b[i00] + c[i00];
        sum[i00] = v;
        sumf += dot(v, v);
    }
    sumf = simd_sum(sumf);
